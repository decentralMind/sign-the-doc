...
contract SignTheDoc {
  ...
  /**
   * @dev Modifier to check if the signature of sender.
   */
   ...
   * `signature`, `authorisedSignerList` and valid ECDSA signature data `r`, `s`
   * and `v`.
   ...
   * - Provide ECDSA signature data `r`, `s` and `v`, generated by signing document hash `docHash`
   *   must be valid.
   */
   ...
   * Signer must provide valid ECDSA `signature` data `r`, `s` and v.
   * Signing the document and recording process is complete through `publishSign` method.
   * Requirements:-
   ...
   * must provide valid EDSCA `signature` data `r`, `s` and `v`.
   * Requirements: -
   * - Signer EDSCA `signature` data `r`, `s` and `v` should be valid.
   *
   * Returns true;
   */
